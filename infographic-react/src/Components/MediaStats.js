...
export default function MediaStats({ people }) {
  return (
    <div className="bg-tertiary rounded-lg p-6 shadow-lg flex flex-col justify-between h-full">
      {/* Title and Icon Section */}
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-white text-center w-full">Media Stats</h2>
        <Image size={32} className="" />
      </div>

      {/* Media Stats Section */}
      <div className="flex flex-col space-y-4 overflow-y-auto flex-grow">
        {people.map(person => (
          <div key={person.name} className=" rounded-lg p-6 flex flex-col justify-between h-full">
